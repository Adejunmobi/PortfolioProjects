...
select location, MAX(total_deaths) as HighestDeathCount, MAX((total_deaths/population)*100) as PercentagePopulationDeath
from CovidDeaths
where continent is not null
Group By Location
order by PercentagePopulationDeath desc
/**because of the data type that the column is saved as it giving aa not accurate
result so we have to CAST the column to covert it to INT doing the following**/
...
/** cast(vaccine.new_vaccinations as int OR
convert(int, vaccine.new_vaccinations) **/
...
--use a CTE, remmeber the number of columns in the WITH and the SELCT must be the same
...
/**to correct or edit the table we can recreate and use the DROP statement
to drop the previously exist one
**/
